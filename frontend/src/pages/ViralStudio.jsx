...
async function callBackendAI({ system, prompt, timeout = 90000, image = null }) {
  ...
  try {
    ...
    if (image?.base64) {
      ...
      payload.image_type = image.type || 'image/jpeg'
    }

    const request = requestMessage => generateBrandAdvisor({ ...payload, message: requestMessage }, {
      timeout,
      signal: controller.signal,
    })

    let response = await request(prompt)
    let result = response?.data?.output || ''
    if (containsPromptLeak(result)) {
      response = await request(`${prompt}\n\nREPAIR REQUIRED: The previous answer copied instructions instead of writing content. Rewrite every section now with final publish-ready words. Do not use square brackets, angle-bracket placeholders, meta-instructions, or phrases such as "write the". Return only completed content.`)
      result = response?.data?.output || ''
    }
    clearTimeout(timeoutId)
    return containsPromptLeak(result) ? '' : result
  } catch (err) {
    clearTimeout(timeoutId)
    throw err
  }
...
8. The format below describes the required fields; never copy its instructional wording into the answer. Every field must contain finished content, not a request to write content.
...
FORMAT: Use the section delimiters EXACTLY as shown above. Do not use square brackets anywhere in the response.`
}
...
function parseViral(text) {
  ...
  // Gemini sometimes returns a generic SECTION NAME delimiter followed by
  // a human-readable heading. Normalize that format for the same cards.
  const aliases = [
    ['hooks', /hook/i],
    ['scripts', /script|visuals/i],
    ['scenes', /scene|shot/i],
    ['captions', /caption/i],
    ['hashtags', /hashtag/i],
    ['music', /music|audio/i],
    ['editing', /edit/i],
    ['growth', /growth|algorithm|posting/i],
  ]
  const genericSections = text.split(/---SECTION NAME---/i).slice(1)
  genericSections.forEach(section => {
    const lines = section.trim().split('\n')
    const heading = lines.shift()?.trim() || ''
    const content = lines.join('\n').replace(/^\s*---\s*/, '').trim()
    const match = aliases.find(([, pattern]) => pattern.test(heading))
    if (match && content) parsed[match[0]] = parsed[match[0]] ? `${parsed[match[0]]}\n\n${content}` : content
  })
  return parsed
}
...
function ImageCaptionTab({ ui, addEntry }) {
  ...
  const handleGenerate = async () => {
    ...
    const prompt = `Analyze the uploaded image as the only source of truth and generate 3 complete, engaging, platform-ready captions for ${platform}.

  Before writing, identify the visible subject, people, objects, setting, colors, mood, readable text, and action. Do not invent details that are not visible. Each caption must mention at least two concrete visual details from this image so it is clearly image-specific.
...
[Image context will be analyzed by AI — no need for descriptions]`
    ...
    try {
      ...
      if (err?.name === 'AbortError' || err?.code === 'ECONNABORTED' || err?.code === 'ERR_CANCELED') {
        setError(ui.errTimeout)
      } else {
        const detail = err?.response?.data?.detail || err?.response?.data?.provider_error || err?.message
        setError(detail ? `${ui.errApi} (${detail})` : ui.errApi)
      }
    }
    setLoading(false)
  }
...
export default function ViralStudio() {
  ...
  const handleViralGenerate = async () => {
    ...
    try {
      const generated = await callBackendAI({
      ...
      const txt = generated || viralFallback(vform)
      if (txt) {
        setVraw(txt)
      ...
      if (err?.name === 'AbortError' || err?.code === 'ECONNABORTED' || err?.code === 'ERR_CANCELED') {
        ...
        const detail = err?.response?.data?.detail || err?.response?.data?.provider_error || err?.message
        setVerr(detail ? `${ui.errApi} (${detail})` : ui.errApi)
      }
    }
    setVload(false)
  }
  ...
  const handleLinkedInGenerate = async () => {
    ...
    try {
      ...
      const txt = generated || linkedInFallback(lform)
      if (txt) {
        setLraw(txt)
      ...
      if (err?.name === 'AbortError' || err?.code === 'ECONNABORTED' || err?.code === 'ERR_CANCELED') {
        ...
        const detail = err?.response?.data?.detail || err?.response?.data?.provider_error || err?.message
        setLerr(detail ? `${ui.errApi} (${detail})` : ui.errApi)
      }
    }
    setLload(false)
  }
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: '1.5rem', alignItems: 'start' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', position: 'sticky', top: '1rem' }}>
              <div style={{ background: 'var(--bg-surface)', border: '1px solid rgba(224,123,90,0.15)', borderRadius: '16px', padding: '1.5rem', boxShadow: 'var(--shadow-sm)' }}>
                ...
                <div style={{ marginBottom: '0.85rem' }}>
                  <label style={lStyle}>{ui.brandTopicLabel}</label>
                  <div style={{ position: 'relative' }}>
                    <input placeholder={ui.brandTopicPlaceholder} value={vform.topic} onChange={vset('topic')} style={{ ...iStyle, paddingRight: '2.8rem' }} />
                    <VoiceInputButton language={vform.language} onText={text => setVform(p => ({ ...p, topic: p.topic ? `${p.topic} ${text}` : text }))} />
                  </div>
                </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', position: 'sticky', top: '1rem' }}>
              <div style={{ background: 'var(--bg-surface)', border: `1px solid rgba(10,102,194,0.18)`, borderTop: `3px solid ${LI_BLUE}`, borderRadius: '16px', padding: '1.5rem', boxShadow: 'var(--shadow-sm)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                  <div>
                    <label style={lStyle}>Topic / Subject *</label>
                    <div style={{ position: 'relative' }}>
                      <input placeholder={isArticle ? 'e.g. Why most founders fail at personal branding' : 'e.g. The mindset shift that doubled my productivity'} value={lform.topic} onChange={lset('topic')} style={{ ...liStyle, paddingRight: '2.8rem' }} />
                      <VoiceInputButton language={lform.language} onText={text => setLform(p => ({ ...p, topic: p.topic ? `${p.topic} ${text}` : text }))} />
                    </div>
                  </div>
...
function VoiceInputButton({ onText, language = 'English' }) {
  ...
  const toggleListening = () => {
    if (!supported) {
      window.alert('Voice input is not supported in this browser. Please use Chrome or Edge.')
      return
    }
    ...
    recognition.lang = VOICE_LANGUAGES[language] || 'en-IN'
  ...
  return (
    <button
      ...
      title={supported ? (listening ? 'Stop voice input' : 'Speak your topic') : 'Voice input requires Chrome or Edge'}
      aria-label={listening ? 'Stop voice input' : 'Start voice input'}
      style={{
        position: 'absolute', right: '8px', top: '50%', transform: 'translateY(-50%)',
        width: '28px', height: '28px', borderRadius: '7px',
        border: `1px solid ${listening ? '#E07B5A' : 'rgba(224,123,90,0.3)'}`,
        background: listening ? 'rgba(224,123,90,0.14)' : 'transparent',
        color: listening ? '#E07B5A' : 'var(--text-muted)', cursor: 'pointer',
...
function containsPromptLeak(text) {
  return /\[(?:write|name|describe|specific|complete|exact|another|full|generate|for each|current|add)\b/i.test(text) ||
    /\b(?:write the complete|write the full|placeholder text|insert your|replace this)\b/i.test(text)
}