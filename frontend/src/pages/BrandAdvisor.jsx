...
function parse(text) {
  ...
  return {
    brandNames: lines(get('BRAND NAMES', 'BRAND NAME', 'NAME SUGGESTIONS')),
    taglines: lines(get('TAGLINES', 'BRAND TAGLINES', 'BRAND TAGLINE')),
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (txt) {
        ...
        const hasParsedSections = Object.values(parsed).some(value =>
          Array.isArray(value) ? value.length > 0 : Boolean(value)
        )
        setRaw(txt)
        setData(parsed)
        setView(hasParsedSections ? 'cards' : 'raw')
        // Save to history
        await addEntry({
    ...
    <AppLayout>
      <div style={{ animation: 'fadeIn 0.4s ease' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '420px 1fr', gap: '1.75rem', alignItems: 'start' }}>
          ...
          {/* RIGHT PANEL */}
{/* RIGHT PANEL */}
<div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', position: 'sticky', top: '1rem', maxHeight: 'calc(100vh - 2rem)', overflowY: 'auto' }}>            {!data && !loading && (
              <>
